...
function renderChapter(){
 ...
 const map=document.getElementById('levelGrid');map.className=`atlas-map atlas-range-${rangeView+1} ${(rewardSourceView||rewardDestView)?'atlas-reward-playing':''}`;map.dataset.routeShape=routeSpec.name;map.dataset.curveProfile=routeSpec.curve;map.setAttribute('aria-label',`${ch.theme}, ${waypoints[rangeView]}, levels ${first} through ${first+9}`);map.innerHTML=`<div class="atlas-scene" aria-hidden="true"><i></i><b></b><em></em></div><svg class="atlas-route-svg" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">${route}</svg>${nodes}`;
...
function decorateCell(cell,lev,r,c){const rock=findAt(lev.rocks,r,c);if(rock){cell.innerHTML='<div class="rock"></div>';return}const nestI=lev.nests.findIndex(n=>n[0]===r&&n[1]===c);if(nestI>=0){const p=lev.pieces[nestI];cell.innerHTML=`<div class="nest" data-pi="${nestI}" style="--piece-color:${COLORS[p.color]}">${suitSvg(p.suit)}</div>`;return}if(findAt(lev.anchors,r,c))cell.innerHTML+='<div class="anchor">'+icon('anchor')+'</div>';const sg=findAt(lev.suitGates,r,c);if(sg)cell.innerHTML+=`<div class="gate suit">${suitSvg(sg[2])}</div>`;const cg=findAt(lev.colorGates,r,c);if(cg)cell.innerHTML+=`<div class="gate color" style="--gate-color:${gateColor(cg[2])}"><span style="width:42%;height:42%;border-radius:50%;background:${gateColor(cg[2])};box-shadow:inset 0 0 0 4px rgba(255,255,255,.55)"></span></div>`;const rail=findAt(lev.rails,r,c);if(rail)cell.innerHTML+=`<div class="rail">${dirSvg(rail[2])}</div>`;const turn=findAt(lev.turners,r,c);if(turn)cell.innerHTML+=`<div class="turner">${turnSvg(turn[2])}</div>`;const sw=findAt(lev.switches,r,c);if(sw)cell.innerHTML+='<div class="switch-tile"></div>';const dr=findAt(lev.doors,r,c);if(dr)cell.innerHTML+=`<div class="door-tile ${(doorMask&(1<<dr[2]))?'open':''}"><div class="door-bars"></div></div>`}
