...
#c2 .toy-logo.title-drop-run span{opacity:1;will-change:transform,opacity;animation:littleHomeTitleDrop 780ms linear var(--drop-delay) 1 both}
@keyframes littleHomeTitleDrop{0%{opacity:0;transform:translateY(-104px) rotate(var(--toy-entry-rot)) scale(.96)}18%{opacity:1}56%{opacity:1;transform:translateY(calc(var(--toy-rest-y) + 8px)) rotate(calc(var(--toy-rest-rot) + .7deg)) scaleX(1.025) scaleY(.96)}75%{transform:translateY(calc(var(--toy-rest-y) - 5px)) rotate(calc(var(--toy-rest-rot) - .4deg)) scaleX(.99) scaleY(1.018)}89%{transform:translateY(calc(var(--toy-rest-y) + 2px)) rotate(var(--toy-rest-rot)) scaleX(1.006) scaleY(.995)}100%{opacity:1;transform:translateY(var(--toy-rest-y)) rotate(var(--toy-rest-rot)) scale(1)}}
#c2 .toy-tagline{display:inline-block;margin:14px 0 0;padding:7px 14px;border-radius:999px;background:rgba(245,222,172,.94);box-shadow:0 3px 0 rgba(187,147,76,.28),0 8px 18px rgba(60,77,88,.08);color:var(--navy);font-family:Inter,ui-rounded,"Avenir Next",system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;font-size:10px;font-weight:900;line-height:1;letter-spacing:.05em;text-transform:none;white-space:nowrap}
@media (prefers-reduced-motion:reduce){#c2 .toy-logo.title-drop-run span{animation:none!important;opacity:1;will-change:auto;transform:translateY(var(--toy-rest-y)) rotate(var(--toy-rest-rot))!important}}
'''
