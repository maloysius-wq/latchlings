...
s=replace_once(s,'<meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover,user-scalable=no" />','<meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover" />','viewport scaling')
s=replace_once(s,'    <div class="mechanic-note" id="mechanicNote"></div>\n    <div class="board-wrap">','    <div class="story-rail-slot" id="storyRailSlot" aria-live="polite"></div>\n    <div class="board-wrap">','story rail slot')
s=replace_once(s,'    <div class="controls"><button class="side-action" id="resetLevelBtn">','    <div class="mechanic-note mechanic-chip" id="mechanicNote"></div>\n    <div class="controls"><button class="side-action" id="resetLevelBtn">','mechanic note below board')
for old,new in [
...
s=replace_once(s,"const reduced=!!(window.matchMedia&&window.matchMedia('(prefers-reduced-motion: reduce)').matches),canAnimate=!!current&&!reduced&&typeof current.animate==='function';","const reduced=effectiveReducedMotion(),canAnimate=!!current&&!reduced&&typeof current.animate==='function';",'screen reduced motion preference')
...
s=replace_once(s,"function dialogueGroups(beat){const out=[],byName=new Map();for(const [name,text] of dialogueLines(beat)){if(!byName.has(name)){const g=[name,[]];byName.set(name,g);out.push(g)}byName.get(name)[1].push(text)}return out}","function dialogueGroups(beat){return dialogueLines(beat).map(([name,text])=>[name,[text]])}",'chronological dialogue groups')
...
start=s.index('function postProcess(){'); end=s.index('\nfunction schedule(){',start)
...
s=replace_once(s,"['Narrator','For the first time, the Waykeeper deliberately builds a route that has never existed before.']","['Narrator','Now the Waykeeper understands why a route with no historical precedent can still be the right route for the Latchlands today.']",'Copperline continuity')
write(p,s)
...
const EARLY_STORY=[{speaker:'Pippa',line:'Breakfast first. The basket missed a stop that worked perfectly yesterday.'},{speaker:'Rowan',line:'The watering marker is off by a whole garden bed. Little Home moved farther overnight.'},{speaker:'Pip',line:'My kite missed the same crossing. That makes two routes, not one bad basket.'},{speaker:'Bramble',line:'Bread from East Sunpetal missed us by the same distance. This is a pattern.'},{speaker:'Tansy',line:'The morning mail brought three notes from neighbors whose crossings shifted too.'},{speaker:'Pippa',line:'I laid today’s stops over yesterday’s map. None of the endpoints line up anymore.'},{speaker:'Rowan',line:'The islands are where they should be. The routes are the part that fell behind.'},{speaker:'Bramble',line:'Five households sent the same kind of report. Leave the next one at Little Home.'},{speaker:'Pippa',line:'Let’s build one temporary circuit from where everyone actually is this morning.'},{speaker:'Tansy',line:'The new circuit works. For the first time today, people are arriving where they meant to.'},{speaker:'Rowan',line:'Another drift check just came in. The repair held, but the islands kept moving.'},{speaker:'Pippa',line:'Then one repair is not enough. We need a way to keep rewriting the route.'},{speaker:'Bramble',line:'The route mailbox is open. Send coordinates, missed stops, and very specific complaints.'},{speaker:'Pip',line:'I found an old Skyway marker under the grass. It points to where this island used to be.'},{speaker:'Tansy',line:'A family across the meadow missed visiting hour when their crossing vanished. This is bigger than errands.'},{speaker:'Rowan',line:'The marker stones still work. Local crews are testing new stops around them now.'},{speaker:'Pippa',line:'The hardware is not broken. The old map is simply describing yesterday’s Latchlands.'},{speaker:'Bramble',line:'With everyone’s reports together, we can fix routes faster than each island can alone.'},{speaker:'Pip',line:'Three more old markers point to where the islands used to be. Yesterday’s map is the problem.'},{speaker:'Tansy',line:'We have our answer: the Skyway works, but its map is old. Now we rebuild Sunpetal for today.'}];
...
start=s.index('<section class="concept" id="c2"'); end=s.index('<section class="concept" id="c3"',start); segment=s[start:end]
needle='</span></div><button class="settings" aria-label="Settings">'; replacement='</span></div><div class="home-tools"><button class="story-home" aria-label="Story &amp; Residents" title="Story &amp; Residents"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 5.5c2.6-.9 5.2-.5 8 1.2v12c-2.8-1.7-5.4-2.1-8-1.2zM20 5.5c-2.6-.9-5.2-.5-8 1.2v12c2.8-1.7 5.4-2.1 8-1.2z"/><path d="M12 6.7v12"/></svg></button><button class="settings" aria-label="Settings">'
if needle not in segment: raise SystemExit('missing c2 settings markup')
segment=segment.replace(needle,replacement,1)
close_needle='</svg></button></div><div class="brand toy-brand">'; close_replacement='</svg></button></div></div><div class="brand toy-brand">'
if close_needle not in segment: raise SystemExit('missing c2 settings wrapper close')
segment=segment.replace(close_needle,close_replacement,1); s=s[:start]+segment+s[end:]
...
p='style400-story-rail-board.css'; s=read(p); marker='/* Board-first progression. The puzzle surface, not scenery behind it, carries the visual journey. */'; pos=s.index(marker); board_tail=s[pos:]
...
.story-level-rail{min-height:62px;display:grid;grid-template-columns:44px minmax(0,1fr) 54px;gap:8px;align-items:center;padding:7px 8px;border:1px solid color-mix(in srgb,var(--railAccent,#6aa357) 34%,#c8b999);border-radius:17px;background:rgba(255,250,238,.9);box-shadow:0 5px 12px rgba(36,58,73,.09);color:var(--railInk,#315742);overflow:hidden}.story-level-rail.daily-rail{grid-template-columns:44px minmax(0,1fr);background:rgba(246,249,242,.94)}
...
#game .board-wrap{flex:0 1 auto;min-height:0}#game .board{width:min(100%,480px,44svh);max-height:none}#game .mechanic-note{display:flex!important;width:min(100%,470px);max-width:470px;align-items:baseline;justify-content:center;gap:7px;margin:6px auto 2px;padding:7px 10px;font-size:13.5px;line-height:1.25;text-align:left;flex:0 0 auto}#game .mechanic-chip-label{flex:0 0 auto;font-size:10px;font-weight:950;letter-spacing:.08em;text-transform:uppercase;color:#55715e}#game .mechanic-chip-copy{font-size:13.5px;font-weight:700;color:#3d596b}#game .controls{width:min(100%,330px);margin:0 auto;grid-template-columns:64px 148px 64px;justify-content:center;gap:9px;padding:6px 0 0}#game .dpad{width:148px;height:148px;margin:auto}#game .side-action{min-height:58px;border-radius:18px;font-size:12px}.side-action svg{width:24px;height:24px}
.latchling.hint-focus{box-shadow:0 0 0 4px #fff,0 0 0 8px #e4ad32,0 7px 12px rgba(22,39,58,.28)!important;animation:hintPiecePulse .7s ease-in-out 3}.dpad-hit.hint-focus{outline:4px solid #e4ad32!important;outline-offset:-6px;border-radius:20px;background:rgba(255,226,135,.18)}@keyframes hintPiecePulse{50%{filter:brightness(1.18) saturate(1.2)}}
.legend-icon svg{width:26px;height:26px;display:block}.legend-suit svg{fill:#1d2d43}.legend-color-dot{width:23px;height:23px;border-radius:50%;background:#4c8ff4;box-shadow:0 0 0 3px #dce9f5 inset}.legend-switch-icon{width:25px;height:25px;border-radius:7px;display:grid;place-items:center;background:#6da8c5;border:2px solid #315d7e}.legend-switch-icon i{width:11px;height:11px;border-radius:50%;background:#edf9fb;border:2px solid #214966}.settings-actions button{min-height:52px}
...
html[data-motion="reduced"] *,html[data-motion="reduced"] *:before,html[data-motion="reduced"] *:after{animation-duration:.01ms!important;animation-iteration-count:1!important;transition-duration:.01ms!important;scroll-behavior:auto!important}html[data-text-size="large"] .story-screen-card,html[data-text-size="large"] .story-briefing,html[data-text-size="large"] .story-current-chapter{font-size:1.08em}
'''
write(p,s)
...
.atlas-chapter-nav{min-height:48px;grid-template-columns:44px minmax(0,1fr) 44px}.atlas-chapter-arrow{width:44px;height:44px;min-width:44px;border-radius:18px}.atlas-region-dots{justify-content:flex-start;gap:2px;overflow-x:auto;scrollbar-width:none;padding:0 2px}.atlas-region-dots::-webkit-scrollbar{display:none}.atlas-region-dot,.atlas-region-dot.active{position:relative;flex:0 0 44px;width:44px;height:44px;border:0;background:transparent;box-shadow:none;border-radius:14px}.atlas-region-dot:before{content:"";position:absolute;left:50%;top:50%;translate:-50% -50%;width:13px;height:13px;border-radius:50%;border:2px solid rgba(255,255,255,.66);background:rgba(28,55,78,.27);box-shadow:0 2px 6px rgba(16,37,56,.12)}.atlas-region-dot.active:before{width:31px;border-radius:9px;background:var(--atlas-accent,#79b95b);border-color:rgba(255,255,255,.84);box-shadow:0 0 0 2px rgba(31,55,70,.08),0 3px 8px rgba(16,37,56,.18)}.atlas-waypoint-tab{min-height:44px;height:44px}
'''
write(p,s)

p='style400-cinematics-dialogue.css'; s=read(p)
s += r'''

/* Pass 1: every cinematic uses the protected chronological dock. */
.cin-ordered-dialogue-dock .cin-opening-bubble{font-size:13px;line-height:1.34}.cin-ordered-dialogue-dock .cin-opening-speaker b{font-size:10px}.cin-ordered-dialogue-dock .cin-opening-speaker small{font-size:9px}.cinematic-stage>.cin-dialogue-layer{display:none!important}@media(max-height:620px){.cin-ordered-dialogue-dock .cin-opening-bubble{font-size:12px;line-height:1.28}}
'''
write(p,s)
