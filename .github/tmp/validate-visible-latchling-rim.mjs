...
const outDir = 'visible-latchling-rim-audit';
fs.mkdirSync(outDir, { recursive: true });
const levels = [1, 5, 13, 20];
const dprs = phase === 'before' ? [1, 3] : [1, 3];
const all = [];
  ...
  for (const level of levels) {
    ...
    if (phase === 'after') {
      for (const p of data) {
        ...
        if ((p.backgroundImage.match(/gradient\(/g)||[]).length < 3) throw new Error(`L${level} P${p.i}: dedicated rim layer missing`);
