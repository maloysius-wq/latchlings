...
function expressionForLine(name,line=''){const t=String(line||'').toLowerCase();if(/shortcut|ridiculous|technically true|perfectly efficient/.test(t))return name==='Bramble'||name==='Pip'?'smug':'surprised';if(/miss|fail|wrong|stale|farther|separat|out of place|closed|lost|vanish|isolat/.test(t))return name==='Tansy'?'curious':'determined';if(/\?|why|what if|notice|spot|find|found|look|marker|map|drift|measure/.test(t))return 'curious';if(/works|saved|home|visit|friend|reach|cheer|together|connected/.test(t))return 'happy';return CHAR_EXPR[name]||'happy'}
...
window.LatchlingsStoryTheme={decorateLevel,enterLevel,show,close,decorFor,iconSvg,autoEligible,featuredFor,characterChip,residentCard,expressionForLine,behaviorFor};
})();
