...
function residentCard(c){return `<div class="story-person story-person-portrait" data-character="${escapeHtml(c.name)}">${characterAvatar(c)}<span class="story-person-copy"><strong>${escapeHtml(c.name)}</strong><span>${escapeHtml(c.role)}. ${escapeHtml(c.voice)}</span><small class="story-person-contribution">${escapeHtml(c.contribution||CHAR_CONTRIB[c.name]||'')}</small></span></div>`}
...
function vignette(meta,featured=[]){const props=decorFor(meta),people=featured.slice(0,2);return `<div class="story-scene-postcard-stamp">${escapeHtml(meta.location)}</div><div class="story-scene-land"></div>${props.map((p,i)=>`<span class="story-scene-prop scene-p${i+1} prop-${p}">${iconSvg(p)}</span>`).join('')}<span class="story-scene-route"></span>${people.map((c,i)=>characterPortrait(c,i,people.length)).join('')}`}
...
function show(level,manual=false){
 if(!STORY)return;
 const meta=STORY.levelMeta(level),chapter=STORY.chapters[meta.chapter-1],overlay=document.getElementById('storyCardOverlay');if(!overlay)return;storyCardLastFocus=document.activeElement;
