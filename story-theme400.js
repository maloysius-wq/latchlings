...
function show(level,manual=false){
 ...
 const title=automatic&&movement?movement.title:meta.title,line=meta.context,goal=movement?movement.question:meta.flavor,more=[automatic&&movement?movement.setup:null,movement?.stakes,manual?meta.flavor:null].filter(Boolean).join(' '),lineResident=(STORY.cast||[]).find(c=>line.includes(c.name))||featured[0]||null;
