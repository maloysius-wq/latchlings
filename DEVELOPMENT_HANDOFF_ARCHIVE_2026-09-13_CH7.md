...
**Deployment / cleanup:** All temporary Chapter 7 workflows and `.github/tmp` helpers are removed. `.github/workflows` is back to only `validate-repository-access-guard.yml`. Cleanup head before this closeout is `c6dc5209e5af5e9c35fad087ec0d3383274327b7`; because that head was created by a direct ref move, it did not itself trigger Actions. The first closeout handoff commit exposed that the permanent repository guard requires the exact recovery sentence in the preflight block; this follow-up restores that required wording without changing product bytes. Verify both GitHub Pages and the permanent repository-access guard succeed on this final closeout head before starting Chapter 8.
